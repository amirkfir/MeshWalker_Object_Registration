...
def calc_accuracy_test(logdir=None, dataset_expansion=None, dnn_model=None, params=None,
  ...
  # Prepare the dataset
  test_dataset, n_items = dataset.tf_mesh_dataset(params, dataset_expansion, mode=params.network_task[0],
                                                  shuffle_size=0, size_limit=params.test_dataset_size_limit, permute_file_names=False,
                                                  must_run_on_all=True, data_augmentation=data_augmentation)

  # If dnn_model is not provided, load it
  if dnn_model is None:
    dnn_model = rnn_object_registration_model.RnnWalkNet_single_line(params, params.n_classes, params.net_input_dim, model_fn, model_must_be_load=True,
  ...
  eval_loss = tf.keras.metrics.MeanSquaredError(name='eval_loss')
  ...
  for _ in tqdm(range(n_iters)):
    for name_, model_ftrs_, labels_ in test_dataset:
      ...
      loss = eval_loss(predictions, tf.squeeze(labels_))
...
if __name__ == '__main__':
  ...
  from train_object_registration import get_params
  utils.config_gpu(1)
  np.random.seed(0)
